...
原生客户端只识别正式 GitHub Release 中的 `native-v*` 标签。例如 `native-v1.2.9` 才会被 Android、Windows、iOS/iPadOS 和桌面端的检查更新识别；普通 `v*` 是 Web 发布，`android-v*` 是 Android 伴侣发布。Android 和 Windows 标签构建必须提供正式签名参数；本地调试 APK、GitHub Actions artifact 或未签名 Windows 包只能用于测试。iOS 真机安装必须通过 Apple 签名和 TestFlight/App Store，不能用模拟器归档代替。
