...
const recurringTasks = await import("../../app/api/recurring-tasks/route.ts");
...
const { dateKeyInZone } = await import("../../app/time-money.js");
...
let pausedSubResponse = await call(subs, "POST", "/api/subscriptions", { body: { ledgerId: L, name: "暂停中的云盘", amount: 199, accountId: cashId, cycle: "每月", category: "娱乐", nextChargeDate: "2026-07-01" } });
check("创建订阅默认未暂停", pausedSubResponse.status === 200 || pausedSubResponse.status === 201, `${pausedSubResponse.status} ${pausedSubResponse.text}`);
const pausedSubId = (await q("SELECT id FROM subscriptions WHERE name='暂停中的云盘'"))[0]?.id;
r = await call(subs, "PATCH", "/api/subscriptions", { body: { id: pausedSubId, ledgerId: L, paused: true } });
check("暂停订阅状态更新成功", r.status === 200 && r.json?.paused === true, `${r.status} ${r.text}`);
const pausedTransactionCount = Number((await q("SELECT COUNT(*) n FROM transactions WHERE title='自动续费 · 暂停中的云盘'"))[0]?.n ?? 0);
await dbmod.processDueSubscriptions(L);
check("暂停期间到期不自动扣款", Number((await q("SELECT COUNT(*) n FROM transactions WHERE title='自动续费 · 暂停中的云盘'"))[0]?.n ?? 0) === pausedTransactionCount, String(pausedTransactionCount));
r = await call(subs, "GET", `/api/subscriptions?ledger=${L}`);
const pausedResponseItem = r.json?.find((item) => item.id === pausedSubId);
check("订阅列表返回暂停状态", r.status === 200 && pausedResponseItem?.isPaused === true, JSON.stringify(pausedResponseItem));
r = await call(subs, "PATCH", "/api/subscriptions", { body: { id: pausedSubId, ledgerId: L, paused: false } });
check("恢复订阅状态成功", r.status === 200 && r.json?.paused === false, `${r.status} ${r.text}`);
await dbmod.processDueSubscriptions(L);
check("恢复订阅不会补扣暂停期间的历史费用", Number((await q("SELECT COUNT(*) n FROM transactions WHERE title='自动续费 · 暂停中的云盘'"))[0]?.n ?? 0) === pausedTransactionCount, String(pausedTransactionCount));
const resumedNextDate = (await q("SELECT next_charge_date d FROM subscriptions WHERE id=?", pausedSubId))[0]?.d;
check("恢复后下次扣款日跳到未来周期", resumedNextDate > dateKeyInZone(new Date(), "Asia/Shanghai"), String(resumedNextDate));
...
r = await call(recurringTasks, "POST", "/api/recurring-tasks", { body: { ledgerId: L, name: "每日交通", amount: 8.5, type: "支出", accountId: cashId, cycle: "每天", category: "交通", nextRunDate: todayKey, reminderDays: 0 } });
check("创建首次执行日为今天的周期任务", r.status === 201, `${r.status} ${r.text}`);
const recurringTaskId = (await q("SELECT id FROM recurring_tasks WHERE ledger_id=? AND name='每日交通'", L))[0]?.id;
await dbmod.processDueRecurringTasks(L);
const recurringTaskTransactions = await q("SELECT id,amount,type,category,occurrence_key FROM transactions WHERE ledger_id=? AND title='周期记账 · 每日交通'", L);
check("到期支出任务生成一条幂等流水", recurringTaskTransactions.length === 1 && recurringTaskTransactions[0].amount === 850 && recurringTaskTransactions[0].type === "支出" && recurringTaskTransactions[0].category === "交通" && recurringTaskTransactions[0].occurrence_key, JSON.stringify(recurringTaskTransactions));
const recurringTaskBalance = (await q("SELECT current_balance balance FROM accounts WHERE id=?", cashId))[0]?.balance;
await dbmod.processDueRecurringTasks(L);
check("重复执行调度器不重复扣款", (await q("SELECT COUNT(*) n FROM transactions WHERE ledger_id=? AND title='周期记账 · 每日交通'", L))[0]?.n === 1 && (await q("SELECT current_balance balance FROM accounts WHERE id=?", cashId))[0]?.balance === recurringTaskBalance, String(recurringTaskBalance));
const incomeRecurringDate = todayKey;
r = await call(recurringTasks, "POST", "/api/recurring-tasks", { body: { ledgerId: L, name: "工资入账规则", amount: 3200, type: "收入", accountId: cashId, cycle: "每月", category: "薪资发放", nextRunDate: incomeRecurringDate, reminderDays: 0 } });
check("创建周期收入规则", r.status === 201, `${r.status} ${r.text}`);
await dbmod.processDueRecurringTasks(L);
const generatedRecurringIncome = await q("SELECT amount,type,income_category_dynamic AS category FROM transactions WHERE ledger_id=? AND title='周期记账 · 工资入账规则'", L);
check("到期收入任务按收入分类入账并增加余额", generatedRecurringIncome.length === 1 && generatedRecurringIncome[0].amount === 320000 && generatedRecurringIncome[0].type === "收入" && generatedRecurringIncome[0].category === "薪资发放", JSON.stringify(generatedRecurringIncome));
const reminderDate = new Date(`${todayKey}T12:00:00Z`);
reminderDate.setUTCDate(reminderDate.getUTCDate() + 2);
const reminderDateKey = reminderDate.toISOString().slice(0, 10);
r = await call(recurringTasks, "POST", "/api/recurring-tasks", { body: { ledgerId: L, name: "每周交通预算", amount: 30, type: "支出", accountId: cashId, cycle: "每周", category: "交通", nextRunDate: reminderDateKey, reminderDays: 3 } });
check("创建带提醒的周期规则", r.status === 201, `${r.status} ${r.text}`);
await call(recurringTasks, "GET", `/api/recurring-tasks?ledger=${L}`);
await call(recurringTasks, "GET", `/api/recurring-tasks?ledger=${L}`);
const recurringReminderCount = Number((await q("SELECT COUNT(*) n FROM system_notifications WHERE ledger_id=? AND title='周期记账提醒' AND message LIKE '每周交通预算 将于 %'", L))[0]?.n ?? 0);
const recurringReminderOccurrences = Number((await q("SELECT COUNT(*) n FROM scheduled_occurrences WHERE ledger_id=? AND source_type='recurring-task-reminder'", L))[0]?.n ?? 0);
check("周期账单提前提醒在重复刷新时只创建一次", recurringReminderCount === 1 && recurringReminderOccurrences === 1, `${recurringReminderCount} notification(s), ${recurringReminderOccurrences} occurrence(s)`);
const recurringNextDate = (await q("SELECT next_run_date date FROM recurring_tasks WHERE id=?", recurringTaskId))[0]?.date;
const { nextRecurringTaskDate } = await import("../../app/recurring-task-core.js");
check("自动执行后下次日期推进一天", recurringNextDate === nextRecurringTaskDate(todayKey, "每天"), String(recurringNextDate));
r = await call(recurringTasks, "GET", `/api/recurring-tasks?ledger=${L}`);
check("周期任务列表不缓存并返回任务状态", r.status === 200 && r.json?.some((item) => item.id === recurringTaskId && item.type === "支出" && item.reminderDays === 0) && r.headers?.get?.("cache-control") === "no-store, private, max-age=0", `${r.status} ${r.text}`);
r = await call(recurringTasks, "PATCH", "/api/recurring-tasks", { body: { id: recurringTaskId, ledgerId: L, paused: true } });
check("周期任务可暂停", r.status === 200 && r.json?.paused === true, `${r.status} ${r.text}`);
await B.prepare("UPDATE recurring_tasks SET next_run_date=? WHERE id=?").bind(todayKey, recurringTaskId).run();
await dbmod.processDueRecurringTasks(L);
check("暂停期间不会生成流水", (await q("SELECT COUNT(*) n FROM transactions WHERE ledger_id=? AND title='周期记账 · 每日交通'", L))[0]?.n === 1, "出现额外流水");
r = await call(recurringTasks, "PATCH", "/api/recurring-tasks", { body: { id: recurringTaskId, ledgerId: L, paused: false } });
check("恢复时跳过暂停期间积压日期", r.status === 200 && r.json?.paused === false && r.json?.nextRunDate > todayKey, `${r.status} ${r.text}`);
r = await call(recurringTasks, "POST", "/api/recurring-tasks", { body: { ledgerId: L, name: "非法周期", amount: 2, type: "支出", accountId: cashId, cycle: "每两周", category: "交通", nextRunDate: todayKey } });
check("周期任务拒绝不支持的频率", r.status === 400, `${r.status} ${r.text}`);
r = await call(recurringTasks, "POST", "/api/recurring-tasks", { body: { ledgerId: L, name: "未知字段", amount: 2, type: "支出", accountId: cashId, cycle: "每月", category: "交通", nextRunDate: todayKey, unexpected: true } });
check("周期任务拒绝未知请求字段", r.status === 400, `${r.status} ${r.text}`);
r = await call(recurringTasks, "POST", "/api/recurring-tasks", { body: { ledgerId: L + 9999, name: "跨账本", amount: 2, type: "支出", accountId: cashId, cycle: "每月", category: "交通", nextRunDate: todayKey } });
check("周期任务不可跨账本写入", r.status >= 400, `${r.status} ${r.text}`);
r = await call(recurringTasks, "DELETE", `/api/recurring-tasks?id=${recurringTaskId}&ledger=${L}`);
const recurringTombstone = await q("SELECT entity_uuid FROM sync_tombstones WHERE entity_type='recurring-task' AND ledger_id=?", L);
check("删除周期任务创建跨设备同步墓碑", r.status === 200 && recurringTombstone.length === 1 && !(await q("SELECT id FROM recurring_tasks WHERE id=?", recurringTaskId)).length, `${r.status} ${r.text}`);
...
const freshVersionAfterDelete = (await q("SELECT updated_at updatedAt FROM installments WHERE id=?", freshId))[0]?.updatedAt;
check("DELETE 无还款分期成功", r.status === 200 && r.json?.duplicate === false, `${r.text} expected=${freshInstallmentVersion} current=${freshVersionAfterDelete}`);
