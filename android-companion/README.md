...
当前伴侣版本为 `1.1.14`，与 Web `1.1.x` 通过仓库根目录的 `release-compatibility.json` 绑定校验；两者不要求版本号相同，但发布时不得绕过该兼容性门禁。
...
2. 在 Neo Ledger“数据中心 → 自动记账连接”点击“生成并复制安卓配置”。手机、电脑和平板只要能访问同一个 Neo Ledger 服务端即可，不要求同一 Wi-Fi。
...
局域网内可以使用 HTTP 地址；NAS、网站或跨网络访问必须使用 HTTPS 域名，或通过可信 VPN 访问。所有设备使用同一个服务端地址、登录身份/自动记账密钥和账本 ID，就会进入同一个账本；服务端按账本修订号让电脑端、手机端和平板端自动感知新增、修改和删除。不同品牌手机可能还需要允许应用后台运行，并关闭针对该应用的省电限制。
...
```bash
git tag android-v1.1.14
git push origin android-v1.1.14
```
