...
export async function POST(request: Request) {
  try {
    ...
    if (
      ...
      !memberId ||
      !amount ||
      !(body.direction === "owesMe" || body.direction === "iOwe")
    )
      throw new Error("平账参数不完整");
    ...
    await db.batch([
      ...
      db.prepare("INSERT INTO transactions (ledger_id,title,amount,type,mood,category,category_dynamic,income_category,income_category_dynamic,account_id,paid_by_member_id,split_with_member_id,split_mode,my_share_percent,currency,original_amount,original_currency,exchange_rate_micros,original_timezone,occurred_at) VALUES (?,'人情平账',?,?,?,?,?,?,?,?,?,?,'人情平账',?,?,?,?,1000000,'Asia/Shanghai',?)")
        .bind(ledgerId, amount, body.direction === "owesMe" ? "收入" : "支出", body.direction === "iOwe" ? "刚需" : null, body.direction === "iOwe" ? "购物" : null, body.direction === "iOwe" ? "购物" : null, body.direction === "owesMe" ? "其它收入" : null, body.direction === "owesMe" ? "其它收入" : null, account.id, body.direction === "owesMe" ? partner.id : me.id, partner.id, body.direction === "owesMe" ? 0 : 100, account.currency, amount, account.currency, occurredAt),
    ]);
